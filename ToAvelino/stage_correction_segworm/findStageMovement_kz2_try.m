function [frames, movesI, locations] = ...
    findStageMovement_kz2(frameDiffs, mediaTimes, locations, delayFrames, fps, win_ceil)
...
while i < length(mediaTimes)
    ...
    %% kezhi update
    nonan_movesI = ~isnan(sum(movesI,2));
    ...
    if i > 3;
        ...
        frameDiffs_cancel(movesI(i-1,1):movesI(i-1,1)+ave_interval+1)=0;
    end
    ...
    %% kezhi:
     ...
     NN = L-1;
     nn = (0:NN)'-NN/2;
     guass_window = win_ceil*exp(-(1/2)*(a*nn/(NN/2)).^2);

...
%for file = dir(main_dir)'
    ...
    if ~isempty(regexp(file.name, '\w*.hdf5', 'ONCE'))
     ...
     %   if  exist(features_mat, 'file')% && ~is_finished
            ...
            % calculate the 
            abs_diff_fra = getFrameDiffVar(masked_image_file);
            ...
            if is_swimming>0
                ...
                moving_frame(:,1) = ((abs_diff_fra_trim>1)&...
...
%             min_fra_ind_match(min_fra_0_ind,5) = min_fra_ind_match(min_fra_0_ind,1);
            ...
            min_fra_ind_match(1,7)=round((min_fra_ind_match(1,2)-csv_ind(1))/(0.94+0.003*mm2_min_ind)+mov_fra_ind(mm1_min_ind));
            for iin = 2: no_nonzero_csv_ind;
                if iin == 95
                    iin
                end

...
cur_file = get(handles.edit1,'String');
...
for  pp = 0:3;
    ...
    try
        ...
        if ~isempty(stage_locations)
            ...
            h5create(skeletons_file, '/stage_movement/wind_weights', size(wind_weights), 'Datatype', 'double', ...
                'Chunksize', size(wind_weights), 'Deflate', 5, 'Fletcher32', true, 'Shuffle', true)
            h5write(skeletons_file, '/stage_movement/wind_weights', wind_weights);

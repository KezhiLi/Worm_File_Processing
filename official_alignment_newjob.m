...
file_txt_ID = fopen('X:\Kezhi\FromAvelino\failed_files.txt','r');
...
main_dir = 'Z:\single_worm\unfinished';
...
for iif = 1:numel(files);     
    ...
    if ~isempty(regexp(file.name, '\w*.hdf5', 'ONCE'))
        ...
        skeletons_file = fullfile(main_dir, strrep(file.name, '.hdf5', '_skeletons.hdf5'));

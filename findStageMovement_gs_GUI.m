function [frames, movesI, locations] = ...
    findStageMovement_gs_GUI(frameDiffs, mediaTimes, locations, delayFrames, fps, wind_weights, hObject, handles, eventdata)
...
%     end
    ...
    if i > 3;
        ...
        frameDiffs_cancel(movesI(i-1,1):min(length(frameDiffs_cancel),floor(movesI(i-1,1)+ave_interval)+1))=0;
    end
    ...
    %% update the searchDiffs, show it in gui:
     ...
     NN = L-1;
     nn = (0:NN)'-NN/2;
     guass_window = wind_weights*exp(-(1/2)*(a*nn/(NN/2)).^2);
     guass_window = guass_window - mean(guass_window );
    
    searchDiffs = (frameDiffs_cancel(startI:endI)).*(guass_window+1)';
...
%    set(get(handles.axes1,'Children'),'ButtonDownFcn',@axes1_ButtonDownFcn);
    ...
    if i == length(mediaTimes)
        ...
        %Kezhi:
        if ~isnan(gOtsuThr)&&(otsuThr<gOtsuThr*0.15)
            isOtsu = 0;
        end
    end
    ...
    if isOtsu
        ...
        % Compute the local thresholds.
            ...
            %% kezhi:
            if sum(frameDiffs(startI:endI)>gOtsuThr)>=3
                % adjust the locao otsuThr accordingly 
                otsuThr = 0.5*(otsuThr+ gOtsuThr);
            else           
    ...
    %%  set handles data
    % indicate otsuThr
    set(handles.text8,'string',num2str(otsuThr));
    % indicate gOtsuThr
    ...
    if isempty(peakI)
        ...
        elseif i > 2 && abs(mediaTimes(i - 2) - lastMoveTime) < ...
            ...
            elseif ~isempty(spareZeroTimeLocation)
                ...
                movesI(end + 1,:) = [0 0];
                timeOff(i-1) = prevPeakI / fps - mediaTimes(i - 1);
                if verbose
                    peaksI = [1 peaksI];
            ...
            % stage movement.
                ...
                movesI(1,1) = 0;
                timeOff(i-1) = prevPeakI / fps - mediaTimes(i - 2);
                if verbose
...
%                set(handles.text18,'string','continuing, please wait');
    ...
    %% kezhi
    % check if the start-end interval is switched
        if movesI(i-1,1)>movesI(i-1,2)
           temp_i2 = movesI(i-1,1);
           movesI(i-1,1) = movesI(i-1,2)-1;
           movesI(i-1,2) = temp_i2+1;
        elseif movesI(i-1,1)== movesI(i-1,2)
            movesI(i-1,2) = min(movesI(i-1,2)+1, length(frameDiffs)); 
        end
    % prevPeakEndI = j - 1;
    prevPeakEndI = movesI(i - 1,2);
    %%

function Gui_Align_rerun(hObject, handles)
%
...
set(handles.text18,'string','re-running');
if isempty(handles.edit19)
    wind_weights = 0;
else
    wind_weights = str2double(get(handles.edit19,'string'));
end
...
try
    ...
    %% try to run the aligment and return empty data if it fails
        ...
        xShift = handles.xyShift(:,1);
        yShift = handles.xyShift(:,2);
